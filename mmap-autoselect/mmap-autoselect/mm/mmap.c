			...
			struct list_head *uf)
{
  ...
  /* Added as part of my Bachelor thesis*/
        /*Is PB-Scheduler making the call ?
         3 Cases : Whole GB contained in the lenght, 2 MB used and only Bytes used, last case is fallback case
         Idea : if we have atleast a gb or 2mb we use MAP_FIXED flag after gaining a virtual base address to allocate 1-3 VMAs overall and return the base addresss.
        We wanna decompose the length into 3 parts : whole
         By that we guarantee a granular mechanism that
        does not contribute to internal fragmentation of the Hugepages and munmap remains compatible. Cause unmap unmaps a virtual region beginning at a base combined with the total length of the allocated
			      ...
			      unsigned long fd, unsigned long pgoff)
{
	...
	if (flags & PB_SCHEDULER) {
                //get intended bitfields, we need them to add them to base address and do the function calls
                unsigned long gb = len & ~(SZ_1GB - 1);
                unsigned long mb2 = (len % SZ_1GB) & ~(SZ_2MB - 1);
                unsigned long bytes = (len % SZ_1GB) % SZ_2MB;
                unsigned long base_address;


                flags = flags &~ PB_SCHEDULER;                          //PB-Scheduler call detected
                if (gb) {
                        unsigned long off = gb;
                        flags = flags | MAP_HUGETLB | MAP_HUGE_1GB;
                        base_address = ksys_mmap_pgoff(addr, gb ,prot,  flags, fd, pgoff);
                        flags = flags &~ MAP_HUGE_1GB &~ MAP_HUGETLB;
                        flags = flags | MAP_FIXED;

                        if(mb2) {
                               
                                flags = flags | MAP_HUGETLB | MAP_HUGE_2MB;
                                ksys_mmap_pgoff(base_address + off, mb2,prot, flags, fd,pgoff);
                                off += mb2;
                                flags = flags &~ MAP_HUGE_2MB &~ MAP_HUGETLB;
                        }
                        if(bytes) {
                               
                                ksys_mmap_pgoff(base_address + off, bytes ,prot, flags, fd,pgoff);
                        }

                        return base_address;
                } else if(mb2) {
			 
                        unsigned long off = mb2;
                        flags = flags | MAP_HUGETLB | MAP_HUGE_2MB;
                        base_address = ksys_mmap_pgoff(addr, mb2,prot, flags, fd,pgoff);
                        flags = flags &~ MAP_HUGE_2MB &~ MAP_HUGETLB;
                        flags |= MAP_FIXED;
                        if(bytes){
                            ksys_mmap_pgoff(base_address + off, bytes, prot, flags, fd, pgoff);
